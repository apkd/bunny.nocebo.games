...
(function (global, factory) {
    ...
    class Backdrop {
        ...
        _getConfig(config) {
            config = Object.assign(Object.assign({}, Default$7), (typeof config === 'object' ? config : {}));
    ...
    const Default$2 = Object.assign(Object.assign({}, Tooltip.Default), { placement: 'right', offset: [0, 8], trigger: 'click', content: '', template: '<div class="popover" role="tooltip">' + '<div class="popover-arrow"></div>' + '<h3 class="popover-header"></h3>' + '<div class="popover-body"></div>' + '</div>' });
    const DefaultType$2 = Object.assign(Object.assign({}, Tooltip.DefaultType), { content: '(string|element|function)' });
    const Event$1 = {
...
lowPerfDevice || (lowPerfDevice = n.deviceMemory !== undefined && n.deviceMemory < 2);
lowPerfDevice || (lowPerfDevice = n.hardwareConcurrency !== undefined && n.hardwareConcurrency < 2);
lowPerfDevice || (lowPerfDevice = "connection" in n && n.connection.saveData === true);
lowPerfDevice || (lowPerfDevice = "connection" in n && (n.connection.effectiveType === "slow-2g" || n.connection.effectiveType === "2g"));
window.addEventListener("DOMContentLoaded", () => {
    const navbarClassList = document.body.querySelector("#mainNav").classList;
    const animateLogoElements = document.body.querySelectorAll(".animate-logo");
    let navbarLowered = null;
    const lowerNavbar = function () {
        console.log(`${window.scrollY} ${window.innerHeight}`);
        if ((navbarLowered === null || navbarLowered) && window.scrollY < window.innerHeight * 0.75) {
            for (const el of animateLogoElements)
                el.classList.add("animate-logo");
            navbarLowered = false;
            navbarClassList.remove("navbar-shrink");
        }
        else if ((navbarLowered === null || !navbarLowered) && window.scrollY > window.innerHeight * 0.75) {
            for (const el of animateLogoElements)
    ...
    if (lowPerfDevice) {
        document.body.querySelector("#discord-parent").remove();
    }
});
let loaded = false;
function TryLoadDiscord() {
    ...
    if (window.scrollY < window.innerHeight)
        return;
    loaded = true;
    document.body.querySelector("#discord").src
        = "https://e.widgetbot.io/channels/909475206190497823/1044358726842331146";
}
if (!lowPerfDevice) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if ("IntersectionObserver" in window) {
        const lazyVideoObserver = new IntersectionObserver((entries, observer) => entries
